...
for sName, Mids in dummy.items():
    for id in Mids:
        ...
        driver.get(IDURL.format(id))
        sleep(8)
        assigned_tests = driver.find_elements_by_xpath(Assigned_tests)
        if len(assigned_tests) == 1:
            test=driver.find_element_by_xpath(Assigned_tests).click()
            try:
                element = driver.find_element_by_xpath(
                    '//td[contains(@class,"defaulttablecell")and contains(text(),"C:")]')
                text = element.text
                print(text)
            except:
                element = driver.find_element_by_xpath(
